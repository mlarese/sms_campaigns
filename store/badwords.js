...
export const actions = {
    load ({dispatch, commit, state}, {id = null, force = true, options = {}}) {
        ...
        if (id === null) {
            return dispatch('api/get', {url: `/campaigns/badwords`, options, debug: false}, root)
                .then(res => {
                    commit('setList', res.data)
                    return res
                })
        } else {
            return dispatch('api/get', {url: `/campaigns/badwords/${id}`, options}, root)
                .then(res => {
      ...
      delete ({dispatch, commit, state}, id) {
        const url = `/campaigns/badwords/${id}`
      ...
      save ({dispatch, commit, state, getters}) {
        ...
        if (getters.isAddMode) {
          return dispatch('api/post', {url: `/campaigns/badwords`, data}, root)
            .then(r => {
          ...
          let id = data.row_id
          return dispatch('api/put', {url: `/campaigns/badwords/${id}`, data}, root)
            .then(r => {
              commit('addRecord', data)
              commit('set$Record', {})
              return r
